...
const TIMEOUT_MS = parseInt(process.env.RESEND_TIMEOUT_MS || '15000', 10);

// fetch with an abort timeout so a hung connection can't stall the removal loop.
...
async function getReceived(id) {
  ...
  try {
    const resp = await fetchWithTimeout(base + encodeURIComponent(id), { headers: { Authorization: 'Bearer ' + key } });
