...
async function logReveal(obj) {
  ...
  if (!ph) return;
  const rec = { phone: String(obj.phone || ''), source: String(obj.source || ''), fields: revealFieldsFrom(obj) };
  let payload;
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        out.push({ phone: rec.phone || '', firstSeen: row.first_seen, source: rec.source || '', fields: rec.fields || {} });
...
module.exports = {
  ...
  getCachedLookup, setCachedLookup, logReveal, listReveals, revealFieldsFrom,
