...
async function getRemovalStats(customerId) {
  let rows;
  if (USE_PG) { const r = await pool.query('SELECT status FROM removal_jobs WHERE customer_id = $1', [customerId]); rows = r.rows; }
  else { rows = sdb.prepare('SELECT status FROM removal_jobs WHERE customer_id = ?').all(customerId); }
  let cleared = 0, inProgress = 0;
  for (const j of rows) {
    if (j.status === 'removed' || j.status === 'no_record') cleared++;
    else inProgress++;
  }
  return { cleared, inProgress, total: rows.length };
}
...
async function recordFunnelEvent(e) {
  ...
  if (USE_PG) {
    await pool.query(
      `INSERT INTO funnel_events (visitor_id, event, email, plan, amount, meta) VALUES ($1,$2,$3,$4,$5,$6)`,
      [visitor, event, email, plan, amount, meta]
    );
  } else {
    sdb.prepare(`INSERT INTO funnel_events (visitor_id, event, email, plan, amount, meta) VALUES (?,?,?,?,?,?)`)
      .run(visitor, event, email, plan, amount, meta);
  }
}

async function getFunnelEventsSince(iso) {
  if (USE_PG) {
    const { rows } = await pool.query('SELECT visitor_id, event, email, plan, amount, created_at FROM funnel_events WHERE created_at >= $1 ORDER BY id ASC', [iso]);
    return rows;
  }
  return sdb.prepare('SELECT visitor_id, event, email, plan, amount, created_at FROM funnel_events WHERE created_at >= ? ORDER BY id ASC').all(toSqliteTs(iso));
}
...
module.exports = {
  ...
  insertReply, listRepliesSince, getRemovalStats,
