...
async function init() {
  if (USE_PG) {
    ...
    // Customer-submitted spam-call reports. The reported number + note are
    // encrypted at rest (note can carry incidental PII). phone_hash is the
    // deterministic hash, kept for future crowd-sourced spam intel.
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS spam_reports_created_idx ON spam_reports (created_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS spam_reports_cust_idx ON spam_reports (customer_id, created_at)`);
    await pool.query(`
    ...
    `);
    // Reveal log: one row per number we've ever shown identity data for. payload
    // is an encrypted JSON { phone, source, fields:{...presence booleans} }.
    // first_seen is preserved across updates so it reads as "first revealed".
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS reveal_log_first_seen_idx ON reveal_log (first_seen)`);
  } else {
    ...
    `);
    sdb.exec(`CREATE INDEX IF NOT EXISTS spam_reports_created_idx ON spam_reports (created_at)`);
    sdb.exec(`CREATE INDEX IF NOT EXISTS spam_reports_cust_idx ON spam_reports (customer_id, created_at)`);
    sdb.exec(`
      CREATE TABLE IF NOT EXISTS lookup_cache (
        phone_hash TEXT PRIMARY KEY,
        payload TEXT NOT NULL,
    ...
    `);
    sdb.exec(`CREATE INDEX IF NOT EXISTS reveal_log_first_seen_idx ON reveal_log (first_seen)`);
  }
  try { await backfillPhoneHashes(); } catch (e) { console.error('[db] phone_hash backfill skipped:', e && e.message); }
  try { await backfillRevealLog(); } catch (e) { console.error('[db] reveal_log backfill skipped:', e && e.message); }
...
async function getCustomerByPhoneHash(hash) {
  ...
  if (USE_PG) {
    const { rows } = await pool.query(
      `SELECT c.* FROM protected_numbers pn JOIN customers c ON c.id = pn.customer_id
       WHERE pn.phone_hash = $1 AND c.status IN ('active','canceling') ORDER BY c.id DESC LIMIT 1`,
      [hash]
    );
    return rowOut(rows[0]);
  }
  return rowOut(sdb.prepare(
    `SELECT c.* FROM protected_numbers pn JOIN customers c ON c.id = pn.customer_id
     WHERE pn.phone_hash = ? AND c.status IN ('active','canceling') ORDER BY c.id DESC LIMIT 1`
  ).get(hash));
}
...
async function deleteCustomersByIds(ids) {
  ...
  const children = ['removal_jobs', 'protected_numbers', 'customer_alerts', 'removal_replies', 'spam_reports'];
  if (USE_PG) {
...
// ---- Lookup cache: one paid people-data call per number, then served from here.
// Keyed by the deterministic phoneHash (no raw number stored); the PII payload is
// encrypted at rest. A missing/corrupt row is treated as a miss and never throws.
async function getCachedLookup(ph) {
  if (!ph) return null;
  try {
    if (USE_PG) {
      const { rows } = await pool.query('SELECT payload FROM lookup_cache WHERE phone_hash = $1', [ph]);
      if (!rows[0]) return null;
      return JSON.parse(decField(rows[0].payload));
    }
    const r = sdb.prepare('SELECT payload FROM lookup_cache WHERE phone_hash = ?').get(ph);
    if (!r) return null;
    return JSON.parse(decField(r.payload));
  } catch (e) {
    return null;
  }
}

async function setCachedLookup(ph, obj) {
  if (!ph || !obj) return;
  let payload;
  try { payload = encField(JSON.stringify(obj)); } catch (e) { return; }
  try {
    if (USE_PG) {
      await pool.query(
        'INSERT INTO lookup_cache (phone_hash, payload) VALUES ($1, $2) ' +
        'ON CONFLICT (phone_hash) DO UPDATE SET payload = EXCLUDED.payload, created_at = now()',
        [ph, payload]
      );
    } else {
      sdb.prepare(
        "INSERT INTO lookup_cache (phone_hash, payload) VALUES (?, ?) " +
        "ON CONFLICT(phone_hash) DO UPDATE SET payload = excluded.payload, created_at = datetime('now')"
      ).run(ph, payload);
    }
  } catch (e) {
    console.error('[db] setCachedLookup error:', e && e.message);
  }
}

// ---- Reveal log: which numbers we've shown identity data for, and what fields.
// Pure: turn a lookupIdentity() result into a compact presence map + its source.
// `name` is attempted by every source; the rest are only attempted by the rich
// people-data source (Enformion), so the API can render "—" when it fell back.
function revealFieldsFrom(obj) {
  obj = obj || {};
  const addr = obj.address || {};
  const hasAddress = (typeof obj.hasAddress === 'boolean')
    ? obj.hasAddress
    : !!(addr.line || addr.city || addr.state || addr.postal);
  return {
    name: !!obj.name,
    age: !!obj.age,
    dob: !!obj.dob,
    ...
    children: !!(obj.children && String(obj.children).trim()),
    address: hasAddress,
    email: !!obj.email,
    relatives: !!(Array.isArray(obj.relatives) && obj.relatives.length),
  };
}

// Pure: turn a lookupIdentity() result into the ACTUAL display values per field
// (empty string when the field is absent). Mirrors revealFieldsFrom's presence
// rules so value-present iff field-present. Stored encrypted; rendered inline in
// the admin reveals table.
function revealValuesFrom(obj) {
  obj = obj || {};
  const addr = obj.address || {};
  ...
  if (hasAddress) {
    ...
    address = [addr.line, cityState, addr.postal].filter(Boolean).join(' · ') || 'Yes';
  }
  let relatives = '';
  if (Array.isArray(obj.relatives) && obj.relatives.length) {
    relatives = obj.relatives.map((r) => (r && (r.name || r.fullName)) || '').filter(Boolean).join(', ')
      || String(obj.relatives.length);
  }
  return {
    name: s(obj.name),
    age: s(obj.age),
    dob: s(obj.dob),
    gender: s(obj.gender),
    language: s(obj.language),
    children: s(obj.children),
    address: address,
    email: s(obj.email),
    relatives: relatives,
  };
...
// Record (or refresh) that we showed identity data for this number. Keyed by the
// deterministic phoneHash; first_seen is preserved across updates so it always
// reads as the first time we revealed. Encrypted at rest. Never throws.
async function logReveal(obj) {
  if (!obj || !obj.ok || !obj.phone) return;
  const ph = phoneHash(obj.phone);
  if (!ph) return;
  const rec = { phone: String(obj.phone || ''), source: String(obj.source || ''), fields: revealFieldsFrom(obj), values: revealValuesFrom(obj) };
  let payload;
  try { payload = encField(JSON.stringify(rec)); } catch (e) { return; }
  try {
    if (USE_PG) {
      await pool.query(
        'INSERT INTO reveal_log (phone_hash, payload, first_seen, updated_at) VALUES ($1, $2, now(), now()) ' +
        'ON CONFLICT (phone_hash) DO UPDATE SET payload = EXCLUDED.payload, updated_at = now()',
        [ph, payload]
      );
    } else {
      sdb.prepare(
        "INSERT INTO reveal_log (phone_hash, payload, first_seen, updated_at) VALUES (?, ?, datetime('now'), datetime('now')) " +
        "ON CONFLICT(phone_hash) DO UPDATE SET payload = excluded.payload, updated_at = datetime('now')"
      ).run(ph, payload);
    }
  } catch (e) {
    console.error('[db] logReveal error:', e && e.message);
  }
}

// List reveals first_seen on/after sinceISO (newest first). Returns decoded rows:
// { phone, firstSeen, source, fields }. A corrupt row is skipped, never thrown.
async function listReveals(sinceISO, limit) {
  ...
  try {
    ...
    if (USE_PG) {
      const r = await pool.query(
        'SELECT payload, first_seen FROM reveal_log WHERE first_seen >= $1 ORDER BY first_seen DESC LIMIT $2',
        [sinceISO || '1970-01-01', lim]
      );
      rows = r.rows;
    } else {
      rows = sdb.prepare(
        'SELECT payload, first_seen FROM reveal_log WHERE first_seen >= ? ORDER BY first_seen DESC LIMIT ?'
      ).all(sinceISO || '1970-01-01', lim);
    }
    for (const row of rows) {
      try {
        const rec = JSON.parse(decField(row.payload));
        out.push({ phone: rec.phone || '', firstSeen: row.first_seen, source: rec.source || '', fields: rec.fields || {}, values: rec.values || {} });
      } catch (e) { /* skip corrupt */ }
    }
  } catch (e) {
    console.error('[db] listReveals error:', e && e.message);
  }
  return out;
}

// One-time fill of reveal_log from the existing lookup_cache (all rich Enformion
// reveals), so history shows up immediately. Idempotent: never overwrites a row
// that's already logged (ON CONFLICT DO NOTHING), and uses the cache's created_at
// as first_seen. No-op once every cached number is represented.
async function backfillRevealLog() {
  let filled = 0;
  const upsert = async (ph, payload, firstSeen) => {
    if (USE_PG) {
      const r = await pool.query(
        'INSERT INTO reveal_log (phone_hash, payload, first_seen, updated_at) VALUES ($1, $2, $3, now()) ' +
        'ON CONFLICT (phone_hash) DO NOTHING',
        [ph, payload, firstSeen]
      );
      return r.rowCount || 0;
    }
    const info = sdb.prepare(
      "INSERT INTO reveal_log (phone_hash, payload, first_seen, updated_at) VALUES (?, ?, ?, datetime('now')) " +
      'ON CONFLICT(phone_hash) DO NOTHING'
    ).run(ph, payload, firstSeen);
    return info.changes || 0;
  };
  let rows;
  if (USE_PG) {
    const r = await pool.query('SELECT phone_hash, payload, created_at FROM lookup_cache');
    rows = r.rows;
  } else {
    rows = sdb.prepare('SELECT phone_hash, payload, created_at FROM lookup_cache').all();
  }
  for (const row of rows) {
    let obj;
    try { obj = JSON.parse(decField(row.payload)); } catch (e) { continue; }
    if (!obj) continue;
    const rec = { phone: String(obj.phone || ''), source: String(obj.source || ''), fields: revealFieldsFrom(obj), values: revealValuesFrom(obj) };
    let payload;
    try { payload = encField(JSON.stringify(rec)); } catch (e) { continue; }
    try { filled += await upsert(row.phone_hash, payload, row.created_at); } catch (e) { /* skip */ }
  }
  if (filled) console.log('[db] reveal_log backfilled for', filled, 'number(s)');
}

// --- spam-call reports (customer-submitted spam numbers) -------------------
// The categories a customer can pick on the dashboard. Anything else is coerced
// to 'Other' so the column stays clean.
const SPAM_CATEGORIES = ['Robocall', 'Scam or fraud', 'Telemarketer', 'Debt or collections', 'Other'];

// Decrypt the phone + note on a spam_reports row.
function spamOut(r) {
  if (!r) return r;
  return Object.assign({}, r, {
    phone: r.phone != null ? decField(r.phone) : r.phone,
    note: r.note != null ? decField(r.note) : r.note,
  });
}

async function insertSpamReport(r) {
  r = r || {};
  const cid = r.customerId;
  if (!cid) return null;
  const phoneRaw = String(r.phone == null ? '' : r.phone).trim();
  if (!phoneRaw) return null;
  const category = SPAM_CATEGORIES.indexOf(r.category) >= 0 ? r.category : 'Other';
  const note = r.note == null ? null : (String(r.note).trim().slice(0, 500) || null);
  const encPhone = encField(phoneRaw);
  const encNote = note ? encField(note) : null;
  const ph = phoneHash(phoneRaw);
  if (USE_PG) {
    const { rows } = await pool.query(
      `INSERT INTO spam_reports (customer_id, phone, phone_hash, category, note) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [cid, encPhone, ph, category, encNote]);
    return spamOut(rows[0]);
  }
  const info = sdb.prepare(`INSERT INTO spam_reports (customer_id, phone, phone_hash, category, note) VALUES (?,?,?,?,?)`)
    .run(cid, encPhone, ph, category, encNote);
  return spamOut(sdb.prepare('SELECT * FROM spam_reports WHERE id = ?').get(Number(info.lastInsertRowid)));
}

// One customer's own reports (for their dashboard list).
async function listSpamReportsForCustomer(customerId, limit) {
  limit = Math.min(Math.max(parseInt(limit || 50, 10), 1), 200);
  let rows;
  if (USE_PG) { const r = await pool.query('SELECT * FROM spam_reports WHERE customer_id = $1 ORDER BY id DESC LIMIT $2', [customerId, limit]); rows = r.rows; }
  else { rows = sdb.prepare('SELECT * FROM spam_reports WHERE customer_id = ? ORDER BY id DESC LIMIT ?').all(customerId, limit); }
  return rows.map(spamOut);
}

async function countSpamReportsForCustomer(customerId) {
  if (USE_PG) { const { rows } = await pool.query('SELECT COUNT(*)::int AS c FROM spam_reports WHERE customer_id = $1', [customerId]); return rows[0] ? rows[0].c : 0; }
  const r = sdb.prepare('SELECT COUNT(*) AS c FROM spam_reports WHERE customer_id = ?').get(customerId);
  return r ? r.c : 0;
}

// Map a joined admin row -> decoded { ...report, cust_email, cust_name }.
function spamAdminOut(r) {
  return Object.assign({}, spamOut(r), {
    cust_email: r.cust_email || '',
    cust_name: r.cust_name != null ? decField(r.cust_name) : '',
  });
}

// Admin: all reports newest-first, with the reporting customer's name/email.
async function listSpamReports(limit) {
  limit = Math.min(Math.max(parseInt(limit || 200, 10), 1), 1000);
  const sql = `SELECT s.*, c.email AS cust_email, c.name AS cust_name FROM spam_reports s
                 LEFT JOIN customers c ON c.id = s.customer_id ORDER BY s.id DESC LIMIT `;
  let rows;
  if (USE_PG) { const r = await pool.query(sql + '$1', [limit]); rows = r.rows; }
  else { rows = sdb.prepare(sql + '?').all(limit); }
  return rows.map(spamAdminOut);
}

// Admin/digest: reports created on/after sinceISO, oldest-first.
async function listSpamReportsSince(iso) {
  const sql = `SELECT s.*, c.email AS cust_email, c.name AS cust_name FROM spam_reports s
                 LEFT JOIN customers c ON c.id = s.customer_id WHERE s.created_at >= `;
  let rows;
  if (USE_PG) { const r = await pool.query(sql + '$1 ORDER BY s.id ASC', [iso]); rows = r.rows; }
  else { rows = sdb.prepare(sql + '? ORDER BY s.id ASC').all(toSqliteTs(iso)); }
  return rows.map(spamAdminOut);
}
...
module.exports = {
  ...
  getCachedLookup, setCachedLookup, logReveal, listReveals, revealFieldsFrom, revealValuesFrom,
  ...
  SPAM_CATEGORIES, insertSpamReport, listSpamReportsForCustomer, countSpamReportsForCustomer, listSpamReports, listSpamReportsSince,
};
