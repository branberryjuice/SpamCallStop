...
async function buildDigest(sinceISO) {
  ...
  let spamReports = [];
  try { spamReports = await db.listSpamReportsSince(sinceISO); } catch (e) { spamReports = []; }
  ...
  L.push('');
  if (spamReports.length) {
    L.push('SPAM CALLS REPORTED BY CUSTOMERS (' + spamReports.length + '):');
    spamReports.slice(0, 30).forEach((s) => {
      const who = (s.cust_name || s.cust_email || ('member #' + s.customer_id));
      L.push('  > ' + (formatPhone(s.phone) || s.phone || '(number)') + '  [' + (s.category || 'Other') + ']  from ' + who);
      if (s.note) L.push('      "' + String(s.note).replace(/\s+/g, ' ').slice(0, 160) + '"');
    });
    L.push('');
  }
  ...
  const spamPart = spamReports.length ? (', ' + spamReports.length + ' spam reports') : '';
  const subject = 'SpamCallStop daily digest: ' + sentRecently + ' sent, ' + replies.length + ' replies, ' + attention.length + ' to review' + spamPart;
  return { subject, text, html, stats: { sent: sentRecently, replies: replies.length, attention: attention.length, spamReports: spamReports.length, byStatus, byKind } };
}
