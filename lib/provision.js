...
async function provisionFromSession(session, opts) {
  ...
  if (sideEffects) {
    try {
      ...
      // Drop a welcome alert so the dashboard shows activity on the first login,
      // not an empty feed. Best-effort; a failure here must not block provisioning.
      try {
        const n = (q && q.brokers) || 0;
        await db.insertAlert({
          customerId: saved.id,
          kind: 'welcome',
          title: 'Protection started',
          body: n
            ? ("We've received your details and filed removals across " + n + " data brokers. You're all set, and we'll keep you posted right here.")
            : "We've received your details and started your removals. You're all set, and we'll keep you posted right here.",
        });
      } catch (ae) { console.error('[provision] welcome alert failed:', ae && ae.message); }
