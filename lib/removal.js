...
const FOLLOWUP_DAYS = parseInt(process.env.REMOVAL_FOLLOWUP_DAYS || '7', 10); // after the 30-day grace, follow up weekly
...
async function processDue(limit) {
  ...
  for (const job of due) {
    ...
    if (r.ok) {
      // First contact gets the full 30-day grace; after that we follow up weekly
      // until the broker confirms removal (or we hit MAX_ATTEMPTS).
      const followup = (job.attempts || 0) > 0;
      const cooldownDays = followup ? FOLLOWUP_DAYS : COOLDOWN_DAYS;
      await db.updateRemovalJob(job.id, {
        status: 'sent', sent_at: new Date().toISOString(),
        next_eligible_at: isoInDays(cooldownDays), attempts: (job.attempts || 0) + 1, last_error: null,
      });
      // Drop an in-app alert so the customer sees activity in their dashboard.
      try {
        await db.insertAlert({
          customerId: job.customer_id,
          kind: followup ? 'removal_followup' : 'removal_sent',
          title: followup ? 'Follow-up sent' : 'Removal request sent',
          body: followup
            ? 'We followed up with a data broker that has not yet confirmed your number was removed.'
            : 'We sent a request to a data broker to remove your number from their lists.',
          brokerKey: job.broker_key,
        });
      } catch (ae) { console.error('[removal] alert insert failed:', ae && ae.message); }
...
const HEARTBEAT_DAYS = parseInt(process.env.REMOVAL_HEARTBEAT_DAYS || '5', 10);

// Parse a DB timestamp (PG Date, or SQLite "YYYY-MM-DD HH:MM:SS" with no zone)
// into millis. Treat a zone-less string as UTC so cadence math is consistent.
function tsMillis(v) {
  if (!v) return 0;
...
// Periodic "we're still working" reassurance. For each protected customer with
// removals still in flight, if we haven't posted ANY alert in HEARTBEAT_DAYS,
// drop a single heartbeat with the live in-progress count. Driven entirely off
// real job state, so it never claims a removal that hasn't been confirmed. The
// "last alert within N days" gate means an active feed (recent sends/replies)
// naturally suppresses the heartbeat — it only fills genuine silence.
async function sendHeartbeats() {
  const out = { checked: 0, sent: 0 };
  let customers = [];
  try { customers = await db.listCustomers(); } catch (e) { return out; }
  const cutoff = Date.now() - HEARTBEAT_DAYS * 86400000;
  for (const c of customers) {
    const status = c.status || 'active';
    if (status !== 'active' && status !== 'canceling') continue; // only the protected
    out.checked++;
    try {
      const recent = await db.listAlertsForCustomer(c.id, 1);
      if (recent && recent[0] && tsMillis(recent[0].created_at) > cutoff) continue; // still has fresh activity
      const stats = await db.getRemovalStats(c.id);
      const n = (stats && stats.inProgress) || 0;
      if (n <= 0) continue; // nothing in flight to report
      await db.insertAlert({
        customerId: c.id,
        kind: 'heartbeat',
        title: 'Still working for you',
        body: 'We have ' + n + ' removal ' + (n === 1 ? 'request' : 'requests')
          + ' in progress. Brokers can take a few weeks to confirm, and we follow up until they do.',
      });
      out.sent++;
    } catch (e) { console.error('[heartbeat] customer', c.id, 'failed:', e && e.message); }
  }
  return out;
}

module.exports = { enqueueForCustomer, processDue, composeEmail, jobIsDue, sendHeartbeats, COOLDOWN_DAYS, MAX_ATTEMPTS, HEARTBEAT_DAYS };
